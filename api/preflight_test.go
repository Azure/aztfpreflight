...
func Test_Preflight(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			RequestUrl: fmt.Sprintf("https://management.azure.com/subscriptions/%s/resourceGroups/myResourceGroup/providers/Microsoft.Storage/storageAccounts/example?api-version=2023-01-01", subscriptionId),
			PayloadJson: `
			{
				"location": "eastus",	
				"kind": "StorageV2",
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			_, err := api.Preflight(context.TODO(), tc.RequestUrl, tc.PayloadJson)
			if tc.ExpectedError == "" {
				if err != nil {
